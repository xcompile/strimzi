...
Kafka Broker is the central point connecting all these components. The broker it self uses on Apache Zookeeper for
storing configuration and for cluster coordination (electing leaders, etc.). Before installing Apache Kafka, Apache
...
Describe installation of HA Kafka broker and the basic configuration.

== Apache Kafka Connect

Describe installation of standalone Kafka Connect and distributed Kafka connect cluster and the basic configuration.

== Apache Kafka Producr and Consumer APIs

Describe how to use Kafka clients (add dependencies to pom.xml).

== Apache Kafka Streams API

Describe how to use Kafka Streams (add dependencies to pom.xml).